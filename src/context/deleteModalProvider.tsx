...
export function DeleteModalProvider({
  ...
  useEffect(() => {
    if (isOpen) {
      setTitle(title.length > 0 ? title : t("ModalTitleDelete"));
      setDescription(
        description.length > 0 ? description : t("ModalDeleteDescription")
      );
      setCancelButtonValue(cancelButtonValue || t("cancel"));
      setConfirmButtonValue(confirmButtonValue || t("delete"));
    }
  ...
  const closeModal = () => {
    ...
    setDescription("");
    setTitle("");
    setConfirmButtonValue(t("delete"));
    setElementId(null);
    setOnDelete(null);
    setTable(undefined);
