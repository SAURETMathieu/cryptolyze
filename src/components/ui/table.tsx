...
const Table = React.forwardRef<HTMLTableElement, TableProps>(
  ({ className, parentClassName, scrollBarClassName, ...props }, ref) => (
    <ScrollArea
      className={cn("h-[64vh] overflow-auto", parentClassName)}
      scrollBarClassName={cn("pt-[61px]", scrollBarClassName)}
    >
      <table
        ref={ref}
...
>(({ className, ...props }, ref) => (
  <td
    ref={ref}
    className={cn(
      "p-4 py-2 align-middle [&:has([role=checkbox])]:pr-0 [&:has([role=decorative])]:w-4 [&:has([role=decorative])]:p-0 [&:has(img)]:pr-0",
      className
    )}
    {...props}
  />
