...
import * as DateTimeRangeFieldPrimitive from "@/src/components/ui/date-time-range-field-primitive";
...
export const DatePickerDateRangeField = React.forwardRef<
  React.ComponentRef<typeof DateTimeRangeFieldPrimitive.Root>,
  Omit<
...
export const DatePickerDateRangeFieldMonths =
...
export const DatePickerCalendar = ({
  ...
  const { mode, month, onMonthChange, value, onValueChange, disabled } =
    useDatePickerContext();

  const Comp = asChild ? (Slot as typeof DayPicker) : DayPicker;

  const modeProps = {
    single: { mode: "single" as const, selected: value as Date },
    multiple: { mode: "multiple" as const, selected: value as Date[] },
    range: { mode: "range" as const, selected: value as DateRange },
  }[mode];
  ...
  return (
    <Comp
      ...
      onSelect={(value: Date | Date[] | DateRange | undefined) => {
        if (!value) {
          if (mode === "single") {
            (onValueChange as (value: Date | null) => void)(null);
          } else if (mode === "multiple") {
            (onValueChange as (value: Date[] | null) => void)(null);
          } else if (mode === "range") {
            (onValueChange as (value: DateRange | null) => void)(null);
          }
          return;
        }
        if (mode === "single") {
          (onValueChange as (value: Date) => void)(value as Date);
        }
        if (mode === "multiple") {
          (onValueChange as (value: Date[]) => void)(value as Date[]);
        }
        if (mode === "range") {
          (onValueChange as (value: DateRange) => void)(value as DateRange);
        }
      }}
      month={month}
