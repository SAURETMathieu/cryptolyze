...
const InputBaseContext = React.createContext<InputBaseContextProps>({
  autoFocus: false,
  controlRef: { current: null as unknown as HTMLElement },
  disabled: false,
  onFocusedChange: () => {},
});
...
>(({ autoFocus, disabled, className, onClick, ...props }, ref) => {
  ...
  return (
    <InputBaseContext.Provider
      value={{
        autoFocus,
        controlRef: controlRef as React.RefObject<HTMLElement>,
        disabled,
    ...
    >
      <Primitive.div
        ...
        className={cn(
          "border-input ring-offset-background flex min-h-10 cursor-text items-center gap-2 rounded-md border bg-transparent px-3 py-2 text-sm",
          disabled && "cursor-not-allowed opacity-50",
          focused && "ring-ring outline-none ring-2 ring-offset-2",
          className
        )}
        {...props}
      />
...
>(({ className, ...props }, ref) => (
  <Primitive.input
    ref={ref}
    className={cn(
      "file:text-foreground placeholder:text-muted-foreground w-full flex-1 bg-transparent file:border-0 file:bg-transparent file:text-sm file:font-medium focus:outline-none disabled:pointer-events-none",
      className
    )}
    {...props}
  />
