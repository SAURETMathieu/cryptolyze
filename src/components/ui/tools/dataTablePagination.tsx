...
const tableSizeOptions = [25, 50, 100, 200, 300];
  ...
  return (
    <div className="flex items-center justify-end px-2 sm:justify-between">
      <div className="text-muted-foreground flex-1 text-left text-sm max-sm:hidden">
        {table.getCenterRows().length} {tTable("of")}{" "}
        {table.getFilteredRowModel().rows.length}{" "}
        {viewMode === "card" ? tTable("element") : tTable("row")}(s).
      </div>
      <div className="flex items-center space-x-2 lg:space-x-6">
        <div className="flex items-center space-x-2">
          <p className="text-sm font-medium max-sm:hidden lg:hidden xl:block">
            {viewMode === "card"
              ? tTable("elementPerPage")
              : tTable("rowPerPage")}
          </p>
          <Select
            value={`${pageSize}`}
          ...
          >
            <SelectTrigger
              ...
              aria-label={tTable("pageSizeAriaLabel")}
            >
        ...
        <div className="flex w-[100px] items-center justify-center text-sm font-medium">
          {tTable("page")} {pageIndex + 1} {tTable("of")} {table.getPageCount()}
        </div>
