...
export function createDealSelectColumn(
  ...
  return {
    ...
    header: ({ table }) => {
      ...
      const selectableRows = allRows.filter((row) => !isDisabled(row));
      ...
      const allNonDisabledRowsSelected = allRows.every(
        (row) =>
          row.getIsSelected() ||
          isDisabled(row) ||
          (firstSelectedRow &&
            firstSelectedRow?.original?.address_id !== row.original.address_id)
      );
    ...
    cell: ({ row, table }) => {
      ...
      const firstSelectedRow = allRows.find((row) => row.getIsSelected());
