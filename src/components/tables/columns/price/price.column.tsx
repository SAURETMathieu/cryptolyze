 ...
 * @param {boolean} [params.withCopy=false] - Indicates if the copy button should be displayed.
  ...
  return {
    ...
    cell: ({ row }) => {
      return (
        <div
          className={cn(
            "ml-auto flex max-w-[85px] justify-end gap-2 text-end font-bold",
            className
          )}
        >
          {priceFn(row)} {devise}
          {withCopy && <CopyButton toCopy={priceFn(row)?.toString()} />}
        </div>
      );
    },
    enableSorting,
