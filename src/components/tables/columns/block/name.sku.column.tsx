 ...
 * @param {function} [params.supplementaryLabelFn] - Function to format the supplementary information.
 *
 * @example
...
export function createNameAndSkuColumn({
  ...
  withCopy?: boolean;
  withCopyName?: boolean;
  supplementaryLabelFn?: (row: string) => string;
}): ColumnDef<any> {
  return {
    id,
    accessorKey,
    ...
    cell: ({ row }) => {
      ...
      const supplementary = supplementaryAccessorKey
        ...
        : null;
      const supplementaryLabel = supplementaryLabelFn
        ? supplementaryLabelFn(supplementary)
        : supplementary;
      return (
        <div
          className={cn(
            "flex max-w-[120px] flex-col gap-1 md:max-w-[200px] lg:max-w-[250px]",
            className
          )}
        >
          <div className="flex items-center gap-2">
            ...
            {withCopyName && name && <CopyButton toCopy={name} />}
          </div>
          <div
            className={cn("flex items-center gap-2", supplementary && "gap-8")}
          >
            <span
              className="text-muted-foreground max-w-[85%] truncate"
              title={sku}
            >
              {sku}
            </span>
            ...
            >
              {supplementaryLabel} {supplementaryText}
            </span>
